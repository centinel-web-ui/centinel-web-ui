...
%>
<%
	String numFiles = request.getParameter("numberfiles1");
...
<!-- Add Plotting Graphs here -->
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            Comparison of Ping Results - TTL Field Values
                        </div>
                        <div class="panel-body">
				<div id="graph"></div>
				<pre id="code" class="prettyprint linenums" hidden>
					Morris.Bar({
					  element: 'graph',
					  data: [
				<% for(int index=1, ttlindex=1;index<numFilesCount+1;index++, ttlindex++) {	
	
						int arrayIndex = 0;
					if(index==numFilesCount){
						out.println("{x: 'ping"+index+"', y: ttl"+ttlindex+"Array["+(arrayIndex)+"], z: ttl"+(ttlindex)+"Array["+(arrayIndex+1)+"], a: ttl"+(ttlindex)+"Array["+(arrayIndex+2)+"],b: ttl"+(ttlindex)+"Array["+(arrayIndex+3)+"]}");
					} else {
						out.println("{x: 'ping"+index+"', y: ttl"+ttlindex+"Array["+(arrayIndex)+"], z: ttl"+(ttlindex)+"Array["+(arrayIndex+1)+"], a: ttl"+(ttlindex)+"Array["+(arrayIndex+2)+"],b: ttl"+(ttlindex)+"Array["+(arrayIndex+3)+"]},");					}
					 ...
					 xkey: 'x',
					  ykeys: ['y','z','a','b'],
					  labels: [hostsArray[0],hostsArray[1],hostsArray[2],hostsArray[3]]
					});
				</pre>		
		</div>		
		</div>		
